...
public class panoCamScript : MonoBehaviour {
	...
	void prepSubCameras() {
		...
		for(int i = 0; i < transform.childCount; i++) {
			...
			child.camera.fieldOfView = 2*Mathf.Atan(hwratio*Mathf.Tan (135*Mathf.Deg2Rad/transform.childCount))*Mathf.Rad2Deg;
			child.localEulerAngles = new Vector3(0, 270.0f/transform.childCount * i, 0);
			child.camera.rect = new Rect(onepart * i, 0, onepart, 1);
		}
	}
}
