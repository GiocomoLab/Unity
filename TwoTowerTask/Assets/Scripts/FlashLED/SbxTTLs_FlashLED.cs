...
public class SbxTTLs_FlashLED : MonoBehaviour
{

    
    private PC_FlashLED pc;
    private int numFlashes_local = -1;
    ...
    void Update()
    {
        ...
        if (numFlashes_local != sp.numFlashes)
        {
            numFlashes_local++;
            if ((dx != 0) | (dy != 0) | (dz != 0))
            {
                if (numFlashes_local > 0)
                {
                    if (numFlashes_local % 2 == 1)
                    {
                        move_laser(dx, dy, dz);
                    }
                    else
